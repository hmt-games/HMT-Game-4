    ...
    */
    ...
    private IEnumerator InitializeMap()
    {
        ...
        if (!_runner.IsServer)
        {
            ...
            MapGenerator.Instance.RPC_RequestNetworkIDs(_runner.LocalPlayer);

            MapGenerator.Instance.mapUpdated = false;
            MapGenerator.Instance.towerIDReceived = false;
            MapGenerator.Instance.floorIDsReceived = false;
            MapGenerator.Instance.gridCellIDsReceived = false;
            MapGenerator.Instance.plantIDsReceived = false;

            // Wait for all data to be received via streaming
            while (!MapGenerator.Instance.mapUpdated)
            {
                yield return null;
            }
        }
    ...
    public void OnReliableDataReceived(NetworkRunner runner, PlayerRef player, ReliableKey key, ArraySegment<byte> data)
    {
        ...
        if (key0 == tKey0 && key1 == tKey1 && key2 == tKey2 && key3 == tKey3)
        {
            ...
            Debug.Log("Deserialize tower: " + MapGenerator.Instance.DeserializeNetworkID(data));
            MapGenerator.Instance.towerID = MapGenerator.Instance.DeserializeNetworkID(data);
            Debug.Log("Received tower ID");
            MapGenerator.Instance.towerIDReceived = true;
        }
        ...
        {
            Debug.Log("Deserialize floor: " + MapGenerator.Instance.DeserializeNetworkIDs(data));
            MapGenerator.Instance.floorIDs = MapGenerator.Instance.DeserializeNetworkIDs(data);
            Debug.Log("Received floor IDs");
            MapGenerator.Instance.floorIDsReceived = true;
        }
        ...
        if (key0 == fKey0 && key1 == fKey1 && key2 == fKey2 && key3 == fKey3)
        {
            Debug.Log("Deserialize Grid cell: " + MapGenerator.Instance.DeserializeGridCellIDs(data, floorCount, gridX, gridY));
            MapGenerator.Instance.gridCellIDs = MapGenerator.Instance.DeserializeGridCellIDs(data, floorCount, gridX, gridY);
            Debug.Log("Received grid cell IDs");
            MapGenerator.Instance.gridCellIDsReceived = true;
        }
        ...
        if (key0 == fKey0 && key1 == fKey1 && key2 == fKey2 && key3 == fKey3)
        {
            Debug.Log("Deserialize plants: " + MapGenerator.Instance.DeserializePlantIDs(data, floorCount, gridX, gridY, plantCount));
            MapGenerator.Instance.plantIDs = MapGenerator.Instance.DeserializePlantIDs(data, floorCount, gridX, gridY, plantCount);
            Debug.Log("Received plant IDs");
            MapGenerator.Instance.plantIDsReceived = true;
        }

        // Update the mapUpdated flag when all IDs are received
        if(MapGenerator.Instance.towerIDReceived && MapGenerator.Instance.floorIDsReceived 
            && MapGenerator.Instance.gridCellIDsReceived && MapGenerator.Instance.plantIDsReceived)
        {
            MapGenerator.Instance.mapUpdated = true;
        }
        Debug.Log("MapGenerator.Instance.mapUpdated: " + MapGenerator.Instance.mapUpdated);
    }
